...
다운받은 노래들 리스트들을 검색해서 추가하는 기능도 들어있습니다.
...
(이미 이전에 다운로드된적 있는 파일이면 바로 플레이리스트에 추가)
...
플레이리스트 버튼 클릭시 다음과 같은 플레이리스트가 출력이 됩니다.
...
이미 다운로드 받은 노래명을 사용해 검색할수도 있습니다. 입력한 문자열을 사용해서 검색해줍니다.
![검색내용](https://drive.google.com/uc?export=view&id=19F7v7BhFaDMVUrl-PoPo2dAF90LB8miF)
검색한 내용은 다음과 같이 버튼으로 표시되며 버튼을 클릭하면 자동으로 노래가 플레이리스트큐에 추가됩니다.
